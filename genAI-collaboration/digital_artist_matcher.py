...
artwork gallery, and chatbot preferences.
"""
...
def format_match_output(match):
    ...
    output = [
        ...
        "",
        "### Why This Artist Is a Good Match:",
    ]
    ...
    for category, score in score_breakdown.items():
        ...
        output.append(f"- **{category_name}:** {score:.1f}")
    
    output.append("")
    output.append("### Primary Tools:")
    primary_tools = artist.get("extracted_tools", {}).get("primary_tools", [])
    output.append(", ".join(primary_tools) if primary_tools else "None specified")
    
    output.append("")
    output.append("### Primary Art Types:")
    primary_art_types = artist.get("extracted_art_types", {}).get("primary_art_types", [])
    output.append(", ".join(primary_art_types) if primary_art_types else "None specified")
    
    output.append("")
    output.append("### Recent Work:")
    gallery = artist.get("completeGallery", [])
    for artwork in gallery[:2]:  # Show only first 2 artworks
        output.append(f"- **{artwork.get('title')}** ({artwork.get('year')}) - {artwork.get('medium')}")
        output.append(f"  {artwork.get('description')[:100]}...")
    
    output.append("")
    output.append(f"**Website:** {basic_info.get('website', 'Not available')}")
    output.append(f"**Contact:** {basic_info.get('email', 'Not available')}")
...
def run_example():
    """
    Run an example of the digital artist collaboration matcher.
    """
    # Create the matcher
    ...
    chatbot_preference = "Looking for digital artists who work in Blender and After Effects for an animated NFT project. Need someone with experience in 3D and motion graphics."
    ...
    # Print the chatbot preference
    print("CHATBOT PREFERENCE:")
    print(chatbot_preference)
    print("\n" + "="*80 + "\n")
    
    # Print the top 3 matches
    print("TOP MATCHES:\n")
    for i, match in enumerate(matches[:3], 1):
        print(format_match_output(match))
        if i < len(matches[:3]):
            print("\n" + "-"*50 + "\n")
