...
export function RouteGuard({ children, requiredUserType }: RouteGuardProps) {
  ...
  useEffect(() => {
    ...
    if (!isLoading) {
      ...
      if (isLoggedIn && requiredUserType && user?.userType !== requiredUserType) {
  ...
  if (isLoading || (requiredUserType && isLoggedIn && user?.userType !== requiredUserType)) {
    return (
  ...
  // Always show content if no specific user type is required or if the user has the correct type
  return <>{children}</>;
}
